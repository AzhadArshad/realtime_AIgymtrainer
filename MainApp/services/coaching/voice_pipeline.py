...
def autoplay_audio(audio_bytes):
    ...
    # Inject <audio> directly into the parent page's document.body.
    # Streamlit only manages its own React subtree, so elements appended
    # to body survive reruns completely — the clip plays to completion.
    components.html(
        f"""
        <script>
            try {{
                var p = window.parent.document;
                var old = p.getElementById('gym-coach-audio');
                if (old) {{ old.pause(); old.remove(); }}
                var a = p.createElement('audio');
                a.id = 'gym-coach-audio';
